...
class World():
  ...
  def draw_fog(self, surface):
        # Tạo một surface trong suốt để làm lớp sương mù
        fog_surface = pygame.Surface((constants.SCREEN_WIDTH, constants.SCREEN_HEIGHT), pygame.SRCALPHA)
        fog_surface.fill((0, 0, 0, 150))  # Màu đen với độ mờ (alpha = 150)

        # Vẽ một vòng tròn trong suốt xung quanh nhân vật chính
        if self.player:
            pygame.draw.circle(fog_surface, (0, 0, 0, 0), self.player.rect.center, 200)  # Bán kính 200

        # Vẽ lớp sương mù lên màn hình
        surface.blit(fog_surface, (0, 0), special_flags=pygame.BLEND_RGBA_SUB)